/**
 * Comprehensive Dynamic Linguistic Translation Engine for Brahui, Urdu, and English
...
// Extensive lexicon for high-coverage offline & fallback translation
export const BRAHUI_LEXICON: Record<string, LexiconWord> = {
  ...
  'read': { arabic: 'خوان', latin: 'khwān', urdu: 'پڑھو', english: 'read', pos: 'verb', root: 'khwān', gloss: 'study' },
...
// Common idiomatic phrases for instant exact match
...
/**
 * Searches the lexicon by looking up English, Urdu, Brahui Arabic, or Brahui Roman words
 */
export function lookupLexicon(rawWord: string): LexiconWord | null {
  ...
  // Iterate to find matches across Urdu, Arabic, and Latin forms
...
/**
 * Dynamically translates any sentence token by token, reordering according to target language SOV/SVO syntax.
 * Guarantees that EVERY unique sentence generates a unique, tailored dynamic translation.
 */
export function dynamicTranslateSentence(
  ...
  // Group elements for SOV syntax re-ordering if target is Brahui or Urdu
  ...
  for (const pw of parsedWords) {
    ...
    if (m) {
      let arabicWord = m.arabic;
      let latinWord = m.latin;
      let urduWord = m.urdu;
      let englishWord = m.english;
      ...
      // Unrecognized word: retain and format safely
      const wordText = pw.clean;
      if (targetLang === 'brahui-arabic' || targetLang === 'urdu') {
        objects.push(wordText);
      } else {
        objectsLatin.push(wordText);
      }
    }
  }
  ...
  if (targetLang === 'brahui-arabic' || targetLang === 'brahui-latin') {
    // Construct SOV: Subject + Object/Complements/Others + Verb
